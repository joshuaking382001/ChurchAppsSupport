...
const MarriageCertificateDetail = ({ search, pickDate }) => {
  ...
  useEffect(() => {
    const result = data.filter((val) => {
      ...
      const matchesSearch = search
        ? val.groomChristianName.toLowerCase().includes(search.toLowerCase())
        : true;
  ...
  const handleView = (id) => {
    ...
    navigate(`/MarriageCertificate/${id}`);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await fetch("http://localhost/api/marriagec.php", {
          method: "DELETE",
          headers: {
